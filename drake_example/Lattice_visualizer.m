...
for i=1:size(bounds,1)
    plot([bounds(i,2)-0.2 bounds(i,2)-0.2], [bounds(i,1)-0.2 bounds(i,3)+0.2], 'k', 'LineWidth',3);
    plot([bounds(i,2)-0.2 bounds(i,4)+0.2], [bounds(i,1)-0.2 bounds(i,1)-0.2], 'k', 'LineWidth',3);
    plot([bounds(i,4)+0.2 bounds(i,4)+0.2], [bounds(i,1)-0.2 bounds(i,3)+0.2], 'k', 'LineWidth',3);
    plot([bounds(i,4)+0.2 bounds(i,2)-0.2], [bounds(i,3)+0.2 bounds(i,3)+0.2], 'k', 'LineWidth',3);
end
ax = gca; ax.YDir = 'reverse';

for i = 1:11 %size(states,1)
    orientation = states(i,1);
    x0 = [traj(i, 1:2) (-1+orientation)*pi/2];
    for j = [0 2 3 4 5 6 7 8]
        % show all mp
        if actions(i) == j
            index = 1;
        else
        ...
        pnts = [];
        while 1
            [x_centers, V] = LoadMotionPrimitives(mp_data, j, index, orientation);
            if(isnan(x_centers))
                break;
            end
        ...
        if actions(i) == j
            patch('Faces',f1,'Vertices',v1,'FaceColor','blue','FaceAlpha',.5);
        else
            patch('Faces',f1,'Vertices',v1,'EdgeColor', [0.8 0.8 0.8], 'FaceColor','blue','FaceAlpha',.01);
        end
    end
end
...
for r = 1:num_robots
    for i=1:size(goals{r},1)
        plot(goals{r}(i,2), goals{r}(i,1), 'o', 'MarkerSize', 22, 'Color', [0,0.5,0]);
...
title(''); %clickableLegend('ref. trajectory', 'Jackal measurements'); 
% grid on;
